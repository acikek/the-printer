...
public class ThePrinterProvider implements IBlockComponentProvider, IDataProvider<PrinterBlockEntity> {
	...
	@Override
	public void appendData(IDataWriter data, IServerAccessor<PrinterBlockEntity> accessor, IPluginConfig config) {
		if (config.getBoolean(ThePrinterWailaPlugin.PRINTER_INFO)) {
			PrinterBlockEntity printer = accessor.getTarget();
			if (!printer.getItems().isEmpty()) {
				data.raw().put("WInput", printer.getItems().get(0).copy().writeNbt(new NbtCompound()));
			}
			if (printer.progress > 0) {
				data.raw().putFloat("WProgress", (float) printer.progress / printer.requiredTicks);
			}
				...
				data.raw().putString("WXP", Text.Serializer.toJson(text));
			}
		}
	}
}
